...
resource "null_resource" "cos_config_download_sap" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "aws  --endpoint-url ${var.cos_config["cos_endpoint_url"]} s3 cp s3://${var.cos_config["cos_bucket_name"]}/${var.cos_config["cos_source_folder_path"]} ${var.cos_config["target_folder_path_local"]} --recursive ",
